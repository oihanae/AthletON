...
def generate_plan_from_profile(p):
    ...
    easy_run = "Cardio Z2 (suave)"
    hiit = "HIIT (intervalos)"
    long_run = "Cardio largo Z2"
    mobility = "Movilidad + core"
    rest = "Descanso / rec. activa"
    ...
    elif "10k" in obj or "media marat" in obj or "marat" in obj:
        tempo = "Tempo (umbral)"
        intervals = "Series (VO2)"
        template = [easy_run, strength, intervals, easy_run, strength, long_run, rest]
        if "media marat" in obj or "marat" in obj: template[2] = tempo
    elif "triatl" in obj:
    ...
    for i in range(7):
        ...
        details = default_details_for_session(title, exp)
        items.append((i, title, details))
    return items
...
def default_details_for_session(title, exp):
    ...
    if "Fuerza" in title:
        if "full body" in title.lower():
            return f"{exp_s}: 3x(Prensa/remo/press) 8-12 reps, 60-90''; core 10'."
        else:
            return f"{exp_s}: Empuje/Tirón/Piernas 3-4x6-10; core 10'."
    if "HIIT" in title: return f"{exp_s}: 8-12 x (1' fuerte / 1' suave)."
...
def insert_workout(user_id, wdate, wtype, duration_min, distance_km, rpe, notes):
    c = get_conn().cursor()
    c.execute(
        "INSERT INTO workouts (user_id, wdate, wtype, duration_min, distance_km, rpe, notes, created_at) "
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
...
def get_workouts(user_id, start=None, end=None):
    ...
    params = [user_id]
    if start:
        query += " AND date(wdate) >= date(?)"; params.append(start.isoformat())
    if end:
        query += " AND date(wdate) <= date(?)"; params.append(end.isoformat())
...
def signup_view():
    ...
    if st.button("Registrarme", type="primary", use_container_width=True):
        if get_user_by_email(email.strip()):
            ...
            try:
                uid = create_user(email.strip(), password, name.strip() or None)
...
def onboarding_view(user_id):
    ...
    if submitted:
        ...
        p = get_profile(user_id)
        plan_items = generate_plan_from_profile(p)
        set_plan(user_id, plan_items)
...
def profile_view(user_id):
    ...
    p = get_profile(user_id)
    if not p:
        st.info("Completa el perfil para personalizar tu plan.")
        onboarding_view(user_id)
        return
    ...
    with st.expander("Datos personales y preferencias", expanded=True):
        with st.form("edit_profile"):
            ...
            if st.form_submit_button("Guardar cambios y regenerar plan", use_container_width=True):
                ...
                plan_items = generate_plan_from_profile(get_profile(user_id))
                set_plan(user_id, plan_items)
    ...
    carbs_g = (kcal * (c/100)) / 4
    prot_g = (kcal * (pr/100)) / 4
    fat_g = (kcal * (fa/100)) / 9
    st.caption(f"≈ {carbs_g:.0f} g CH / {prot_g:.0f} g PROT / {fat_g:.0f} g GRAS/día")
...
def log_workout_view(user_id):
    ...
    col1, col2 = st.columns(2)
    with col1:
        duration = st.number_input("Duración (min)", min_value=0.0, step=5.0)
    with col2:
...
def history_view(user_id):
    ...
    col1, col2 = st.columns(2)
    with col1:
        start = st.date_input("Desde", value=date.today() - timedelta(days=30))
    with col2:
    ...
    ax1.plot(agg["week"], agg["duration_min"], marker="o")
    ax1.set_title("Minutos entrenados por semana")
    ax1.set_xlabel("Semana"); ax1.set_ylabel("Minutos")
    st.pyplot(fig1)

    if "distance_km" in df and df["distance_km"].notna().any():
        agg2 = df.groupby("week")["distance_km"].sum().reset_index()
        fig2, ax2 = plt.subplots()
        ax2.bar(agg2["week"].astype(str), agg2["distance_km"])
        ax2.set_title("Kilómetros por semana")
        ax2.set_xlabel("Semana"); ax2.set_ylabel("Km")
        st.pyplot(fig2)
...
def insights_view(user_id):
    ...
    if df.empty:
        st.info("Registra al menos 1-2 semanas para ver insights."); return
    df["week"] = df["wdate"].dt.to_period("W").apply(lambda r: r.start_time.date())
    ...
    if AI_ENABLED:
        ...
        txt = ai_coach_response("Resume el progreso y da 3 recomendaciones accionables.", prof, df)
        st.write(txt)
    else:
        st.caption("IA no configurada (añade OPENAI_API_KEY para habilitarla).")
...
def main():
    ...
    if needs_onboarding(user["id"]):
        onboarding_view(user["id"]); return
    ...
    with tab3:
        history_view(user_id=user["id"])
        st.divider(); insights_view(user_id=user["id"])
    ...
    with tab5:
        ...
        if st.button("Preguntar", type="primary"):
            ...
            ans = ai_coach_response(q, prof, df_last)
            st.write(ans)
